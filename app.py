...
# Configuração de caminhos para estilos locais
...
# Lista de cores VIVAS compatíveis com folium.Icon (Pins do mapa)
CORES_FOLIUM = [
...
with st.sidebar:
    ...
    # 🛠️ FUNCIONALIDADE: TOGGLE DE ATUALIZAÇÃO AUTOMÁTICA
    st.markdown('<div class="section-title" style="margin-top:20px;">Atualização</div>', unsafe_allow_html=True)
    auto_refresh = st.toggle("Atualização Automática", value=True, help="Ativa a busca por novos dados em intervalos fixos.")
    
    if auto_refresh:
        refresh_rate = st.select_slider("Intervalo (seg)", options=[30, 60], value=30)
        st_autorefresh(interval=refresh_rate * 1000, key="datarefresh")
    else:
        st.caption("Atualização automática desligada.")
    ...
    if linha_alvo != "🔍 Selecione uma linha...":
        ...
        if ordens_sidebar:
            filtro_ordens = st.multiselect("Ordens", options=ordens_sidebar, default=[], placeholder="Todos os veículos", label_visibility="collapsed")
        else:
            filtro_ordens = []
    else:
        filtro_ordens = []
...
if dados_full is not None and not dados_full.empty:
    ...
    with tab_mapa:
        ...
        for i, (ordem, trajetoria) in enumerate(dados_bus.groupby('ordem')):
            ...
            # Marcador de INÍCIO (Ícone Play branco com fundo da cor da linha)
            folium.Marker(coords[0], icon=folium.Icon(color='white', icon_color=cor, icon='play', prefix='fa'), tooltip=f"Início ID: {ordem}").add_to(m)

            # Marcador ATUAL (Ícone Ônibus com cor sincronizada com o rastro)
            ultima = trajetoria.iloc[-1]
            folium.Marker([ultima['latitude'], ultima['longitude']], icon=folium.Icon(color=cor, icon='bus', prefix='fa'), tooltip=f"<b>Atual: {ordem}</b>").add_to(m)
    ...
    with tab_ordens:
        linha_busca = st.text_input("Consulta de Linha", placeholder="Digite a linha...")
        if st.button("Buscar"):
